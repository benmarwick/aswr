...
# inspect the output of extract_tables()
as_data_frame(table_from_pdf[[1]])
```

The  tabulizer package also has an interactive mode allowing you to click and drag a rectangle over a page to select the area to extract from. For large PDF files you can use the `split_pdf()` function to separate out the pages with tables to improve the performance of `extract_tables()`.
