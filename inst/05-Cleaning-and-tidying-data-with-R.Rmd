...
In the above example we show the use of the period and asterix as metacharacters in a regular expression. The table below summarises other metacharacters and symbols that are commonly used in regular expressions during data cleaning.

```{r echo = FALSE}
regex_tbl <- readr::read_csv("data/regex.csv")
1+8
knitr::kable(regex_tbl)
```
