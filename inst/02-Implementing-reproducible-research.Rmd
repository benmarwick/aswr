...
Choosing software to write plain text documents can be challenging because of the wide variety of options. I recommend RStudio because it contains many comprensive support for writing R Markdown that save time and effort. For example, RStudio will highlight markdown text and R code in a way that makes it more readable. It will automatically indent or tidy up your code as you write it. It will also passively signal to you when you’ve done something wrong in your code (such as forget a closing bracket or semicolon or quotation mark). RStudio also comes with Pandoc built-in, which is the software that helps with converting between Markdown and Micrsoft Word, PDF and HTML. Having Pandoc built-in to RStudio saves time working with Pandoc directly. 
...
### Code blocks, cross-referencing figures and tables, and inline R code
...
In this example the label is 'simple-addition', and this is the key to make cross-references work in R Markdown. Code block labels have a few constraints, they can only contain alphanumeric characters (a-z, A-Z, 0-9), slashes (/), or dashes (-). Underscores are not allowed in code block labels, and code block labels must be unique, you cannot have two blocks with the same label. We use the labels to link in-text cross-reference to the output of the code block with that label. The general for of cross-referencing is to type `\@ref(fig:label)` where `label` is the code block label that you are referring to. If you are referring to a code block that produces a table (by using `knitr::kable()`), then the general form is `\@ref(tab:label)`. In addition to a code block label, a second element is required to enable cross-referencing. This is a caption, which we can add using the `fig.cap` option in the first line of the code block.  For example, this code block produces a simple plot, and is labelled 'simple-plot', and we have set the caption as 'A simple plot' :

````r
`r ''````{r simple-plot, echo = FALSE, fig.cap = "A simple plot"}
plot(1:10)
```
````

```{r simple-plot, echo = FALSE, fig.cap = "A simple plot"}
plot(1:10)
```

When we want to refer to that plot in our text, we type `\@ref(fig:simple-plot)` and this will be rendered into the figure number that this plot has in our document, for example, Figure \@ref(fig:simple-plot). 

Often in writing for research we want to include some numeric output within our text, such as the output of some data analysis or a hypothesis test. For writers using Microsoft Word or similiar software, they will compute their result in another program, such as Microsoft Excel or SPSS, and then paste the results into their Word document. When writing in R Markdown document, we can write R code in the middle of a sentence to compute a result, and when the document is rendered the code is excecuted and numerical output appears in place of the R code. Inline R code is demarcated with a single backtick at the start and end, and the letter 'r' to indicate the programming language. Here is a simple example: in our R Markdown document we type `` `r '\x60r  5 * 2\x60'` `` (as in 'five times two'), and then when we render that document we will see `r 5 * 2` in place of the R code. 

When writing in line R code it is good practice to keep the code short so that the code doesn't interrupt the flow of the sentence for the reader (i.e. you, when you are writing the Rmd document, and your colleagues who are also editing that document). An easy way to keep the inline code short is to use a code block for the more complex operates, and then assign the result that you want to show in the text to an object. Here iss a simple code block to demonstrate this approach:
...
And here is our narrative text that includes inline code to use the output from that code block in a sentence. The inline code is typed as: `` `r '\x60r  x\x60'` ``, and we can use the output like this: The sum of all the numbers from 1 to 10 is `r x`. This appoach is especially convienent for extracting a few values value from a large table or other data object, and rounding them so it is suitable to include in a sentence. 
